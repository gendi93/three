...
export class PropertyTile {
  ...
  constructor({type, name, description, color, cost, houseCost = 0, rentalValues = []}: PropertyProps) {
  ...
  collectRent = (player: Player, modifier: number) => {
    ...
    if (this.propertyType === PropertyType.Building) {
      console.log(`${player.name} paid ${this.owner.name} £${this.rentalValues[this.houses]} in rent`);
      player.pay(this.rentalValues[this.houses], this.owner);
    } else if (this.propertyType === PropertyType.Station) {
      const ownedStations = this.owner.properties.filter(property => property.propertyType === PropertyType.Station);
      console.log(`${player.name} paid ${this.owner.name} £${this.rentalValues[ownedStations.length - 1]} in travel tickets`);
      player.pay(this.rentalValues[ownedStations.length] * modifier, this.owner);
    } else if (this.propertyType === PropertyType.Utility) {
      const ownedUtilities = this.owner.properties.filter(property => property.propertyType === PropertyType.Utility);
      console.log(`${player.name} paid ${this.owner.name} £${this.rentalValues[ownedUtilities.length - 1] * player.previousRoll.total * modifier} in utility bills`);
      player.pay(player.previousRoll.total * this.rentalValues[ownedUtilities.length] * modifier, this.owner);
    }
  }
}
